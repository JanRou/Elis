...
using ED.Wp3.Server.BE.PrognosisMetadata.Model.Json;
...
namespace ED.Wp3.Server.BE.PrognosisMetadata.Model
{
    public interface IModel
    {
        ...
        Json.Component ToJson(IComponent component);
    ...
    public class Model : IModel
    {
        ...
        public Json.Component ToJson()
        {
            return ToJson(root);
        }

        public Json.Component ToJson(IComponent component)
        {
            Json.Component result;
            if ((component.Type == ComponentType.Compo) 
                || (component.Type == ComponentType.CompoInterceptor) )
            {
                if (component.Type == ComponentType.CompoInterceptor)
                {
                    component.Create();
                }
                Json.Composite jsoncomp = new Json.Composite();
                IComposite composite = (IComposite) component;
                jsoncomp.Name = composite.Name;
                jsoncomp.Type = composite.Type;
                foreach (IComponent comp in composite)
                {
                    Json.Component child = ToJson(comp);
                    jsoncomp.Children.Add(child.Name, child);
                }
                result = jsoncomp;
            }
            else // Leaf
            {
                Json.Leaf jsonleaf = new Leaf();
                ILeaf leaf = (ILeaf) component;
                jsonleaf.Type = leaf.Type;
                jsonleaf.Name = leaf.Name;
                jsonleaf.Item = leaf.Item;
                result = jsonleaf;
            }
            return result;
        }
