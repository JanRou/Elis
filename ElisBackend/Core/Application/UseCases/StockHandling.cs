        ...
        , IMapper mapper) : IStockHandling
    {
        ...
        private async Task<int> GetOrAddTimeSeries(string isin, string timeSeriesName)
        {
            ...
            else
            {
                ...
                if (stockId != 0)
                {
                    var timeSeriesDao = new TimeSeriesDao() { StockId = stockId, Name = timeSeriesName };
                    result = await timeSeriesRepository.AddTimeSeries(timeSeriesDao);
                }
            }
        ...
        private (bool, string) ValidateDates(TimeSeries timeSeries)
        {
            ...
            if (!isOk)
            {
                status = "Error, dates are no consecutive with latest date first";
            }
            return (isOk, status);
        }
        ...
        {
            // TODO Do this validation in GraphQL
            (bool isOk, string status) = (true, "Ok");
