...
namespace ED.Atlas.Service.IC.BE.ModelHandlers.CompositeModel {
    public class Model : IModel {
        ...
        /// <summary>
        /// Sets interceptor on leaf node pointed out ofrelative path from a lookup node.
        /// </summary>
        /// <typeparam name="T">Base type of interceptor.</typeparam>
        ...
        public bool SetInterception<T>(Composite rootNode, string path, IInterceptor<T> interceptor) {
            ...
            bool ret = (node != null) 
                && ( (typeof(Leaf<T>) == node.GetType()) || (typeof(LeafMap) == node.GetType()) ); // Contract coding please!
            if(ret) {
        ...
        private IEnumerable<TreeString> innerToTreeString( Component component, int level) {
            ...
            if (component.Type == ComponentTypeType.Compo) {
                ...
                LeafMap leafMap = component as LeafMap;
                if (leafMap != null) {
                    Map map = leafMap.Value;
                    foreach (IMapEntry entry in map) {
                        yield return
                            new TreeString() {Level = level + 1, Name = entry.Key, Value = entry.Value.ToString()};
                    }
                }
                else {
                    Leaf leaf = (Leaf) component.ToComponentType(); // TODO why going this way in order to get content of a leaf
                    yield return
                            new TreeString() { Level = level + 1, Name = leaf.Name, Value = leaf.Item.ToString() };
                }
            }
        }
